 ...
 * Series colours come from CSS tokens so each theme has its own validated pair
 * (palette checker: light #0A4FD1 / #B0700E on white, dark #5B8CFF / #B7802A on #121B33
 * pass lightness, chroma, colour-blind separation and contrast). Blue = received, gold = resolved.
 */
export const SERIES_1 = 'rgb(var(--series-1))';
export const SERIES_2 = 'rgb(var(--series-2))';
  ...
  return (
    <Card className="flex flex-col p-5">
      <div className="mb-4 flex items-start justify-between gap-3">
        ...
        {table && (
          <button onClick={() => setAsTable((v) => !v)} className="grid h-8 w-8 shrink-0 place-items-center rounded-lg text-ink-500 hover:bg-sunken no-print"
      ...
      {data.map((d) => {
        ...
        const body = (
          <>
            ...
            <div className="h-2.5 w-full rounded-r-[4px] bg-sunken" style={{ maxHeight: 24 }}>
          ...
          <li key={d.key}>
            {d.href
              ? <button onClick={() => nav(d.href!)} title={`${d.key}: ${d.value} ${valueLabel}${secondaryLabel && d.secondary != null ? `, ${d.secondary} ${secondaryLabel}` : ''} (open list)`}
                  className="-mx-2 block w-[calc(100%+16px)] rounded-lg px-2 py-1 text-left hover:bg-sunken focus-visible:bg-sunken">{body}</button>
    ...
    <div className="relative">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full" role="img" aria-label="Grievances by year">
        {ticks.map((t) => {
          ...
          return <g key={t}><line x1={padL} x2={W} y1={y} y2={y} style={{ stroke: GRID }} strokeWidth={1} /><text x={padL - 8} y={y + 4} textAnchor="end" className="fill-ink-500 text-[11px]">{Math.round(t).toLocaleString()}</text></g>;
        })}
        {data.map((d, i) => {
          ...
          return (
            <g key={d.key} onMouseEnter={() => setHover(i)} onMouseLeave={() => setHover(null)} onClick={() => d.href && nav(d.href)} className={d.href ? 'cursor-pointer' : ''}>
              ...
              <path d={roundedTop(x, y, bw, h, 4)} style={{ fill: SERIES_1 }} opacity={hover === null || hover === i ? 1 : 0.45} />
      ...
      {hover !== null && data[hover] && (
        <div className="pointer-events-none absolute right-0 top-0 rounded-lg bg-[#1A1F36] px-2.5 py-1.5 text-xs text-white shadow-raised">
  ...
  return (
    <div className="relative">
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full" role="img" aria-label="Grievances received and resolved per month"
        ...
        {[0, max / 2, max].map((t) => <g key={t}><line x1={padL} x2={W - padR} y1={y(t)} y2={y(t)} style={{ stroke: GRID }} /><text x={padL - 8} y={y(t) + 4} textAnchor="end" className="fill-ink-500 text-[11px]">{Math.round(t)}</text></g>)}
        {months.map((m, i) => (data.length <= 12 || i % 2 === 0) && <text key={i} x={x(i)} y={H - 8} textAnchor="middle" className="fill-ink-500 text-[11px]">{m}</text>)}
        <path d={area} style={{ fill: SERIES_1 }} opacity={0.1} />
        <path d={path('a')} fill="none" style={{ stroke: SERIES_1 }} strokeWidth={2} strokeLinejoin="round" strokeLinecap="round" />
        <path d={path('b')} fill="none" style={{ stroke: SERIES_2 }} strokeWidth={2} strokeLinejoin="round" strokeLinecap="round" />
        {hover !== null && <line x1={x(hover)} x2={x(hover)} y1={8} y2={H - padB} style={{ stroke: 'rgb(var(--ink-400))' }} strokeWidth={1} />}
        {[{ k: 'a' as const, c: SERIES_1 }, { k: 'b' as const, c: SERIES_2 }].map(({ k, c }) => (
          <g key={k}>
            {(hover !== null ? [hover] : [last]).map((i) => <circle key={i} cx={x(i)} cy={y(data[i][k])} r={4.5} style={{ fill: c }} strokeWidth={2} className="stroke-surface" />)}
          </g>
        ))}
        {hover === null && <>
      ...
      {hover !== null && (
        <div className="pointer-events-none absolute left-12 top-0 rounded-lg bg-[#1A1F36] px-3 py-2 text-xs text-white shadow-raised">
...
export function Kpi({ label, value, hint, onClick, tone = 'neutral', icon }: {
  ...
  return (
    <Tag onClick={onClick} className={cx('group flex flex-col rounded-2xl bg-surface p-4 text-left shadow-card transition-shadow',
      onClick && 'hover:shadow-raised focus-visible:shadow-halo', tone === 'danger' && 'ring-2 ring-inset ring-danger/30')}>
      <span className="flex items-center gap-1.5 text-[13px] font-bold text-ink-500">{icon}{label}</span>
      <span className={cx('mt-1.5 text-[1.6rem] font-extrabold leading-tight tabular',
